manhattanVAAST<-function(n.features, sig.level, vaast.simple, title, sig.line=TRUE, axis.text=TRUE, custom.xlab="index", sig.hjust=0, sig.vjust=0){
...
plot(y=-log10(as.numeric(as.character(dat$V3))), x=dat$relative.pos, col=as.factor(dat$seqid), xaxt="n", xlab=custom.xlab, pch=20, ylab="-log10(p-value)", main=title)
